// 远通二线QC报表独立JS
// 仅服务于 yuantong2_report.html 和 yuantong2_report_history.html
// 保证表头与数据顺序严格一致，避免错位
...
(function() {
    ...
    if (typeof window.exportToExcel === 'function') {
        ...
        // 定义远通二线报表的导出函数
        window.exportYuantong2ReportToExcel = async function() {
            console.log('远通二线报表 exportYuantong2ReportToExcel 被调用');
            ...
            try {
                ...
                // 构建导出URL
                const exportUrl = `/yuantong2_report/export_excel/?${params.toString()}`;
                ...
                if (typeof globalExportToExcel === 'function') {
                    ...
                    globalExportToExcel(exportUrl, 'filterForm', 'export', '远通二线QC报表');
                } else {
                    ...
                    if (isWeChatPC) {
                        ...
                        performWeChatWorkExport(exportUrl, 'export', '远通二线QC报表');
                    } else {
                        // 回退到原有方式
        ...
        window.exportToExcel = async function() {
            console.log('导出Excel包装函数被调用，转发给 exportYuantong2ReportToExcel');
            return await window.exportYuantong2ReportToExcel();
        };
        
    } else {
        console.log('未检测到全局exportToExcel函数，将直接定义远通二线报表导出函数');
        // 直接定义远通二线报表的导出函数
        window.exportToExcel = async function() {
            console.log('远通二线报表 exportToExcel 被调用');
...
document.addEventListener('DOMContentLoaded', function() {
    // 判断是否为历史页面（路径包含 /yuantong2_report/history/）
    if (window.location.pathname.includes('/yuantong2_report/history/')) {
        // 支持筛选表单自动加载
        const filterForm = document.getElementById('filterForm');
        if (filterForm) {
            filterForm.addEventListener('submit', function(e) {
                e.preventDefault();
                loadYuantong2HistoryData(1);
            });
        }
        // 页面初次加载时自动加载第一页
        setTimeout(() => loadYuantong2HistoryData(1), 100);
    }
    // 表单页面初始化
    if (window.location.pathname === '/yuantong2_report/') {
        // 初始化计算逻辑
...
function initCalculationLogic() {
    ...
    if (materialTypeSelect) {
        ...
        materialTypeSelect.addEventListener('change', function() {
            ...
            const yuantong2CakeDensityBefore = document.getElementById('yuantong2_cake_density')?.value;
            ...
            console.log('切换前 - 远通二线饼密度:', yuantong2CakeDensityBefore);
            ...
            setTimeout(() => {
                ...
                const yuantong2CakeDensityAfter = document.getElementById('yuantong2_cake_density')?.value;
                const changfuCakeDensityAfter = document.getElementById('changfu_cake_density')?.value;

                console.log('切换后 - 饼密度:', wetCakeDensityAfter);
                console.log('切换后 - 远通二线饼密度:', yuantong2CakeDensityAfter);
    ...
    // 为相关字段添加事件监听器
    const calculationFields = ['yuantong2_permeability_coefficient', 'yuantong2_sample_weight', 'yuantong2_filter_area', 'cake_thickness', 'water_viscosity', 'filter_time'];
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (result.status === 'success') {
                        displayYuantong2HistoryData(result.data);
                ...
                if (page === 1) {
                    logViewOperation();
                        ...
                        updateYuantong2Pagination(result.current_page, result.total_pages, result.total_count);
                    } else {
...
// 系数参数
let coefficients = {
    yuantong2_permeability: 6.4,
    yuantong2_sample_weight: 5,
    yuantong2_filter_area: 3.14
};
...
async function loadCoefficients() {
    try {
        // 加载远通二线渗透率系数
        const response1 = await fetch('/api/report-parameters/yuantong2_permeability_coefficient/');
        const result1 = await response1.json();
        if (result1.status === 'success') {
            coefficients.yuantong2_permeability = parseFloat(result1.data.value);
        }

        // 加载远通二线样品重量
        const response2 = await fetch('/api/report-parameters/yuantong2_sample_weight/');
        const result2 = await response2.json();
        if (result2.status === 'success') {
            coefficients.yuantong2_sample_weight = parseFloat(result2.data.value);
        }

        // 加载远通二线过滤面积
        const response3 = await fetch('/api/report-parameters/yuantong2_filter_area/');
        const result3 = await response3.json();
        if (result3.status === 'success') {
            coefficients.yuantong2_filter_area = parseFloat(result3.data.value);
        }
...
function updateCoefficientsByMaterialType() {
    ...
    if (materialType === '助熔煅烧品') {
        ...
        // 从后台参数管理读取值
        const coefficientField = document.getElementById('yuantong2_permeability_coefficient');
        const sampleWeightField = document.getElementById('yuantong2_sample_weight');
        const filterAreaField = document.getElementById('yuantong2_filter_area');

        if (coefficientField) coefficientField.value = coefficients.yuantong2_permeability || '';
        if (sampleWeightField) sampleWeightField.value = coefficients.yuantong2_sample_weight || '';
        if (filterAreaField) filterAreaField.value = coefficients.yuantong2_filter_area || '';

        console.log('助熔煅烧品系数设置完成:', {
            coefficient: coefficients.yuantong2_permeability,
            sampleWeight: coefficients.yuantong2_sample_weight,
            filterArea: coefficients.yuantong2_filter_area
        });
...
function updateFieldEditability() {
    ...
    const yuantong2CakeDensityField = document.getElementById('yuantong2_cake_density');
    ...
    if (materialType === '助熔煅烧品') {
        console.log('处理助熔煅烧品模式');
        // 助熔煅烧品：远通二线饼密度，长富饼密度两个字段不可编辑
        if (yuantong2CakeDensityField) {
            console.log('清空远通二线饼密度字段');
            yuantong2CakeDensityField.readOnly = true;
            yuantong2CakeDensityField.style.backgroundColor = '#f5f5f5';
            yuantong2CakeDensityField.title = '此字段由系统自动计算';
            // 清空远通二线饼密度和长富饼密度
            yuantong2CakeDensityField.value = '';
        }
        ...
        if (yuantong2CakeDensityField) {
            console.log('启用远通二线饼密度字段编辑');
            yuantong2CakeDensityField.readOnly = false;
            yuantong2CakeDensityField.style.backgroundColor = '';
            yuantong2CakeDensityField.title = '';
        }
        if (changfuCakeDensityField) {
...
function calculateAllValues(skipAutoFill = false) {
    ...
    // 获取输入值
    const yuantong2Coefficient = parseFloat(document.getElementById('yuantong2_permeability_coefficient')?.value) || 0;
    const yuantong2SampleWeight = parseFloat(document.getElementById('yuantong2_sample_weight')?.value) || 0;
    const yuantong2FilterArea = parseFloat(document.getElementById('yuantong2_filter_area')?.value) || 0;
    ...
    console.log('计算参数:', {
        yuantong2Coefficient,
        yuantong2SampleWeight,
        yuantong2FilterArea,
        cakeThickness,
        waterViscosity,
        filterTime
...
function calculateForFluxCalcined() {
    ...
    // 远通二线渗透率 = 远通二线渗透率系数 * 饼厚 * 水黏度 / 过滤时间
    let yuantong2Permeability = null;
    if (yuantong2Coefficient && cakeThickness && waterViscosity && filterTime) {
        yuantong2Permeability = (yuantong2Coefficient * cakeThickness * waterViscosity / filterTime);
        document.getElementById('permeability').value = yuantong2Permeability.toFixed(4);
    } else {
        document.getElementById('permeability').value = '';
    }

    // 长富渗透率 = (远通二线渗透率 - 0.366) / 1.23
    if (yuantong2Permeability !== null) {
        const changfuPermeability = (yuantong2Permeability - 0.366) / 1.23;
    ...
    // 饼密度 = 远通二线样品重量 / 远通二线过滤面积 / 饼厚
    if (yuantong2SampleWeight && yuantong2FilterArea && cakeThickness) {
        const cakeDensity = yuantong2SampleWeight / yuantong2FilterArea / cakeThickness;
        document.getElementById('wet_cake_density').value = cakeDensity.toFixed(3);
    } else {
        document.getElementById('wet_cake_density').value = '';
    }

    // 远通二线饼密度和长富饼密度不可编辑，由系统计算
    // 这里可以根据需要设置固定值或其他计算逻辑
    document.getElementById('yuantong2_cake_density').value = '';
...
function calculateForCalcined(skipAutoFill = false) {
    ...
    // 长富渗透率 = 远通二线渗透率 - 0.02
    if (yuantong2Permeability !== null) {
        const changfuPermeability = yuantong2Permeability - 0.02;
        document.getElementById('permeability_long').value = changfuPermeability.toFixed(4);
    } else {
        document.getElementById('permeability_long').value = '';
    }
    ...
    // 饼密度计算
    // 远通二线饼密度 = 远通二线样品重量 / 远通二线过滤面积 / 饼厚
    // 在煅烧品模式下，如果饼厚为空，则使用默认值1进行计算
    if (yuantong2SampleWeight && yuantong2FilterArea) {
        const effectiveCakeThickness = cakeThickness || 1; // 如果饼厚为空，使用默认值1
        const yuantong2CakeDensity = yuantong2SampleWeight / yuantong2FilterArea / effectiveCakeThickness;
        console.log('计算出的饼密度值:', yuantong2CakeDensity);
        console.log('使用的饼厚值:', effectiveCakeThickness);

        // 如果skipAutoFill为true，则不自动填充饼密度字段
        if (!skipAutoFill) {
            console.log('自动填充饼密度字段');
            document.getElementById('wet_cake_density').value = yuantong2CakeDensity.toFixed(3);
        } else {
            console.log('跳过饼密度字段自动填充（skipAutoFill=true）');
        }

        // 注意：在煅烧品模式下，远通二线饼密度和长富饼密度字段是可编辑的
        // 当参数变化时，自动重新计算这些字段
        const yuantong2CakeDensityField = document.getElementById('yuantong2_cake_density');
        const changfuCakeDensityField = document.getElementById('changfu_cake_density');
        ...
        if (!skipAutoFill) {
            console.log('处理远通二线饼密度和长富饼密度字段');
            // 自动计算并填充远通二线饼密度和长富饼密度
            if (yuantong2CakeDensityField) {
                console.log('自动填充远通二线饼密度字段');
                yuantong2CakeDensityField.value = yuantong2CakeDensity.toFixed(3);
            }
            if (changfuCakeDensityField) {
                const changfuCakeDensity = yuantong2CakeDensity - 0.02;
                console.log('自动填充长富饼密度字段');
                changfuCakeDensityField.value = changfuCakeDensity.toFixed(3);
            }
        } else {
            console.log('跳过远通二线饼密度和长富饼密度字段自动填充（skipAutoFill=true）');
        }
    } else {
...
async function loadYuantong2HistoryData(page = 1, pageSize = currentPageSize) {
    const filterForm = document.getElementById('filterForm');
    const params = new URLSearchParams();
    if (filterForm) {
        const formData = new FormData(filterForm);
...
function displayYuantong2HistoryData(data) {
    ...
    data.forEach(item => {
        ...
        if (canEdit) {
            tds += `<button class="btn btn-sm btn-primary" onclick="editYuantong2Record(${item.id})" title="编辑记录"><span class="material-icons" data-icon="edit">edit</span> 编辑</button>`;
        } else {
        ...
        if (canDelete) {
            tds += `<button class="btn btn-sm btn-danger" onclick="deleteYuantong2Record(${item.id})" title="删除记录"><span class="material-icons" data-icon="delete">delete</span> 删除</button>`;
        } else {
...
function updateYuantong2Pagination(currentPage, totalPages, totalCount) {
    ...
    if (currentPage > 1) {
        paginationHTML += `<button onclick="loadYuantong2HistoryData(${currentPage - 1}, ${currentPageSize})">上一页</button>`;
    }
    const startPage = Math.max(1, currentPage - 2);
    const endPage = Math.min(totalPages, currentPage + 2);
    if (startPage > 1) {
        paginationHTML += `<button onclick="loadYuantong2HistoryData(1, ${currentPageSize})">1</button>`;
        if (startPage > 2) {
    ...
    for (let i = startPage; i <= endPage; i++) {
        if (i === currentPage) {
            ...
            paginationHTML += `<button onclick="loadYuantong2HistoryData(${i}, ${currentPageSize})">${i}</button>`;
        }
    }
    if (endPage < totalPages) {
        ...
        paginationHTML += `<button onclick="loadYuantong2HistoryData(${totalPages}, ${currentPageSize})">${totalPages}</button>`;
    }
    if (currentPage < totalPages) {
        paginationHTML += `<button onclick="loadYuantong2HistoryData(${currentPage + 1}, ${currentPageSize})">下一页</button>`;
    }
    paginationHTML += '</div>';
    ...
    document.getElementById('pageSizeSelect').addEventListener('change', function() {
        ...
        loadYuantong2HistoryData(1, currentPageSize);
    });
    document.getElementById('gotoPageBtn').addEventListener('click', function() {
        ...
        if (page >= 1 && page <= totalPages) {
            loadYuantong2HistoryData(page, currentPageSize);
        }
    });
    // 支持回车跳页
...
function editRecord(id) {
    window.location.href = `/yuantong2-report-edit/${id}/`;
}
async function deleteRecord(id) {
    if (!confirm('确定要删除这条记录吗？')) return;
    try {
        const response = await fetch(`/api/yuantong2-report/${id}/`, {
            method: 'DELETE',
            headers: {
                'X-CSRFToken': getCSRFToken()
            }
        ...
        if (response.ok) {
            alert('删除成功');
            loadYuantong2HistoryData(1); // 刷新数据
        } else {
...
//             console.log('使用增强版导出功能');
//             // 调用qc_report_common.js中的增强导出函数
//             window.exportToExcel(exportUrl, 'filterForm', 'export', '远通二线QC报表');
//         } else {
//             console.log('增强版导出功能不可用，使用回退方式');
    ...
    try {
        ...
        fetch(exportUrl)
            ...
            .then(blob => {
                ...
                const link = document.createElement('a');
                link.href = url;
                link.download = '远通二线QC报表_' + new Date().toISOString().split('T')[0] + '.xlsx';
                link.style.display = 'none';
...
function performSimpleExportForWeChat(exportUrl) {
    ...
    try {
        const fileName = '远通二线QC报表_' + new Date().toISOString().split('T')[0] + '.xlsx';
...
function editYuantong2Record(id) {
    window.location.href = `/yuantong2-report-edit/${id}/`;
}

async function deleteYuantong2Record(id) {
    if (!confirm('确定要删除这条记录吗？此操作不可恢复。')) return;
    const apiUrl = `/api/yuantong2-report/${id}/`;
    try {
        const response = await fetch(apiUrl, {
            method: 'DELETE',
            headers: {
...
async function calculateYesterdayProduction() {
    try {
        ...
        // 调用API获取昨日产量统计
        const response = await fetch('/api/yuantong2-report/?action=yesterday_production', {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    try {
        const response = await fetch('/api/log-view-operation/', {
            ...
            body: JSON.stringify({
                ...
                operation_type: 'VIEW',
                operation_detail: '查看远通二线QC报表历史记录',
