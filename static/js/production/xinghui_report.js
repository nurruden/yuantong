document.addEventListener('DOMContentLoaded', function() {
    // 判断是否为历史页面
    if (window.location.pathname.includes('/xinghui_report/history/')) {
        // 初始化历史页面的日期时间选择器
    ...
    if (form) {
        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch('/api/xinghui-reports/', {
                    method: 'POST',
                    headers: {
...
async function loadXinghuiHistoryData(page = 1, pageSize = 10) {
    currentPage = page;
    pageSize = pageSize;
    ...
    const apiUrl = `/api/xinghui-report/?${params.toString()}`;
    try {
        ...
        if (response.ok) {
            ...
            if (result.status === 'success') {
                displayXinghuiHistoryData(result.data);
                ...
                // 标准分页渲染
                updateXinghuiPagination(
                    result.current_page || 1,
                    result.total_pages || 1,
                    result.total_count || 0,
                    function(page, pageSize) {
                        loadXinghuiHistoryData(page, pageSize);
                    },
                    pageSize
                );
            } else {
...
function displayXinghuiHistoryData(data) {
    ...
    data.forEach(item => {
        ...
        if (canEdit) {
            tds += `<button class="btn btn-sm btn-primary" onclick="editXinghuiRecord(${item.id})" title="编辑记录"><span class="material-icons" data-icon="edit">edit</span> 编辑</button>`;
        } else {
        ...
        if (canDelete) {
            tds += `<button class="btn btn-sm btn-danger" onclick="deleteXinghuiRecord(${item.id})" title="删除记录"><span class="material-icons" data-icon="delete">delete</span> 删除</button>`;
        } else {
...
function updateXinghuiPagination(currentPage, totalPages, totalCount) {
    ...
    if (currentPage > 1) {
        paginationHTML += `<button onclick="loadXinghuiHistoryData(${currentPage - 1}, ${currentPageSize})">上一页</button>`;
    }
    const startPage = Math.max(1, currentPage - 2);
    const endPage = Math.min(totalPages, currentPage + 2);
    if (startPage > 1) {
        paginationHTML += `<button onclick="loadXinghuiHistoryData(1, ${currentPageSize})">1</button>`;
        if (startPage > 2) {
    ...
    for (let i = startPage; i <= endPage; i++) {
        if (i === currentPage) {
            ...
            paginationHTML += `<button onclick="loadXinghuiHistoryData(${i}, ${currentPageSize})">${i}</button>`;
        }
    }
    if (endPage < totalPages) {
        ...
        paginationHTML += `<button onclick="loadXinghuiHistoryData(${totalPages}, ${currentPageSize})">${totalPages}</button>`;
    }
    if (currentPage < totalPages) {
        paginationHTML += `<button onclick="loadXinghuiHistoryData(${currentPage + 1}, ${currentPageSize})">下一页</button>`;
    }
    paginationHTML += '</div>';
    ...
    document.getElementById('pageSizeSelect').addEventListener('change', function() {
        ...
        loadXinghuiHistoryData(1, currentPageSize);
    });
    document.getElementById('gotoPageBtn').addEventListener('click', function() {
        ...
        if (page >= 1 && page <= totalPages) {
            loadXinghuiHistoryData(page, currentPageSize);
        }
    });
    // 支持回车跳页
...
function editXinghuiRecord(id) {
    window.location.href = `/xinghui-report-edit/${id}/`;
}

async function deleteXinghuiRecord(id) {
    if (!confirm('确定要删除这条记录吗？此操作不可恢复。')) return;
    const apiUrl = `/api/xinghui-report/${id}/`;
    try {
        const response = await fetch(apiUrl, {
            method: 'DELETE',
            headers: {
